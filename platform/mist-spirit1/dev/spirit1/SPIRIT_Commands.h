...
typedef enum
{
  ...
  CMD_IQC_INIT_LOAD = COMMAND_IQC_INIT_LOAD,      /*!< Load the IQC word in IQC_CORRECT_OUT [2:0] from IQC_CORRECT_IN[2:0] registers; it is used to let the IQ correction parameters (for image rejection) evolve from packet to packet instead of restarting at each packet; valid from all states (TBC) */
...
/**
 * @brief  Sends the IQC_INIT_LOAD command to SPIRIT. Loads the IQC word in IQC_CORRECT_OUT [2:0] from IQC_CORRECT_IN[2:0] registers.
 * @param  None.
 * @retval None.
 */
#define SpiritCmdStrobeIqcInitLoad()   SpiritCmdStrobeCommand(CMD_IQC_INIT_LOAD)
