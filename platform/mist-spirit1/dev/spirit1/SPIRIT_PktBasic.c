...
void SpiritPktBasicInit(PktBasicInit* pxPktBasicInit)
{
  ...
  for(i=0;i<4;i++){
    if(i<3-(pxPktBasicInit->xSyncLength >>1))
      ...
      tempRegValue[6+i] = (uint8_t)(pxPktBasicInit->lSyncWords>>(8*i));
  }
  ...
  /* Writes registers */
  g_xStatus = SpiritSpiWriteRegisters(PCKTCTRL4_BASE, 10, tempRegValue);
...
void SpiritPktBasicGetInfo(PktBasicInit* pxPktBasicInit)
{
  ...
  for(i=0 ; i<4 ; i++){
      if(i>2-(pxPktBasicInit->xSyncLength >>1))
        pxPktBasicInit->lSyncWords |= tempRegValue[i+6]<<(8*i);
  }
