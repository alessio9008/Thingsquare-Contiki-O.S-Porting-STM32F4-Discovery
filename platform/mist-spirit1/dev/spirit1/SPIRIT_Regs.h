...
#define	PCKT_FLT_OPTIONS_DEST_VS_BROADCAST_ADDR_MASK		((uint8_t)0x02) /*!< Packet discarded if destination address differs from MULTICAST register [RX] */
...
#define	SYNTH_CONFIG0_BASE					((uint8_t)(0x9E)) /*!< Enable DSM randomizer [7], Window width 1.2-7.5ns (Down-up) of lock detector*/
...
#define	COMMAND_IQC_INIT_LOAD			                ((uint8_t)(0x6E)) /*!< Load the IQC word in IQC_CORRECT_OUT [2:0] from IQC_CORRECT_IN[2:0] registers; it is
