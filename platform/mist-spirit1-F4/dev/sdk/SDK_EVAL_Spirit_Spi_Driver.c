...
#define CS_TO_SCLK_DELAY  0x0002
...
void SpiritSpiInit(void)
{
  ...
  RCC_APB1PeriphClockCmd(SPIRIT_SPI_PERIPH_RCC, ENABLE);
  
  /* Enable SCLK, MOSI, MISO and CS GPIO clocks */
  RCC_AHB1PeriphClockCmd(SPIRIT_SPI_PERIPH_MOSI_RCC, ENABLE); 
  RCC_AHB1PeriphClockCmd(SPIRIT_SPI_PERIPH_MISO_RCC, ENABLE);
  RCC_AHB1PeriphClockCmd(SPIRIT_SPI_PERIPH_SCLK_RCC, ENABLE);
  RCC_AHB1PeriphClockCmd(SPIRIT_SPI_PERIPH_CS_RCC, ENABLE);
  
  GPIO_InitStructure.GPIO_Pin =  SPIRIT_SPI_PERIPH_MOSI_PIN |SPIRIT_SPI_PERIPH_MISO_PIN | SPIRIT_SPI_PERIPH_SCLK_PIN; 
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE); //AGGIUNTA
  ...
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2; //era 4; SPI_BaudRatePrescaler_8; /*Baud_Rate= fpclk/4=32MHz/4=8MHZ*/
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
  SPI_InitStructure.SPI_CRCPolynomial = 7;
  SPI_Init(SPIRIT_SPI_PERIPH_NB, &SPI_InitStructure);
  
  SPI_SSOutputCmd(SPIRIT_SPI_PERIPH_NB,ENABLE);//aggiunta
...
SpiritStatus SpiritSpiWriteRegisters(uint8_t cRegAddress, uint8_t cNbBytes, uint8_t* pcBuffer)
{
  ...
  SpiritSPICSLow();
  {
    ...
    for(iv = 0; iv < CS_TO_SCLK_DELAY*20; iv++);
  }  
