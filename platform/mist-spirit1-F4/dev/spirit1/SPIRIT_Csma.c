...
void SpiritCsmaInit(CsmaInit* pxCsmaInit)
{
  ...
  s_assert_param(IS_SPIRIT_FUNCTIONAL_STATE(pxCsmaInit->xCsmaSeedReloadMode));
  ...
  /* Writes the new value for seed reload mode */
  if(pxCsmaInit->xCsmaSeedReloadMode==S_ENABLE)
    tempRegValue[4] |= PROTOCOL1_SEED_RELOAD_MASK;
  else
    tempRegValue[4] &= ~PROTOCOL1_SEED_RELOAD_MASK;
 ...
 void SpiritCsmaGetInfo(CsmaInit* pxCsmaInit)
 {
   ...
   /* Reads the seed reload mode enable bit */
   pxCsmaInit->xCsmaSeedReloadMode = (SpiritFunctionalState)((tempRegValue[4]>>3) & 0x01);
