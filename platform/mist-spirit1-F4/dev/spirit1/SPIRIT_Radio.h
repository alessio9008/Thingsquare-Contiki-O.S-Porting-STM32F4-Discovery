 ...
 * passed argument in dBm because the function performs a linear approximation
 * given by the linearization formula <i>pa[dBm] = PA_UPPER_LIMIT - PA_COEFFICIENT_DBM*PA_LEVEL_x</i>
 * (where <i>PA_COEFFICIENT_DBM</i> is defined in <i>SPIRIT_Config.h</i>).
 *
...
typedef struct
{
  uint32_t          lXtalFrequency;     /*!< Specifies the XTAL frequency. This parameter
                                             is a uint32_t. */
  ...
  float             fChannelSpace;      /*!< Specifies the channel spacing expressed in Hz.
  ...
  float             fDatarate;          /*!< Specifies the datarate expressed in bps.
                                             This parameter can be in the range between
                                             100 bps and 500 kbps */

  float             fFreqDev;           /*!< Specifies the frequency deviation expressed in Hz.
...
#define HIGH_BAND_LOWER_LIMIT         779000000   /*!< Lower limit of the high band: 779 MHz */
#define HIGH_BAND_UPPER_LIMIT         956500000   /*!< Upper limit of the high band: 915 MHz */
#define MIDDLE_BAND_LOWER_LIMIT       387000000   /*!< Lower limit of the middle band: 387 MHz */
#define MIDDLE_BAND_UPPER_LIMIT       470000000   /*!< Upper limit of the middle band: 470 MHz */
#define LOW_BAND_LOWER_LIMIT          300000000   /*!< Lower limit of the low band: 300 MHz */
#define LOW_BAND_UPPER_LIMIT          348000000   /*!< Upper limit of the low band: 348 MHz */
#define VERY_LOW_BAND_LOWER_LIMIT     150000000   /*!< Lower limit of the very low band: 150 MHz */
#define VERY_LOW_BAND_UPPER_LIMIT     174000000   /*!< Upper limit of the very low band: 174 MHz */
...
#define IF_OFFSET(F_Xo)			(lroundf(480000.0/(F_Xo)*12288-64.0))      /*!< It represents the IF_OFFSET in order
...
#define CHANNEL_SPACE_STEP(F_Xo)	        (F_Xo/32768)
#define CHANNEL_SPACE_UPPER_LIMIT(F_Xo)		(CHANNEL_SPACE_STEP(F_Xo)*255)

#define IS_CHANNEL_SPACE(CHANNELSPACE, F_Xo)    (CHANNELSPACE>0 && CHANNELSPACE<=CHANNEL_SPACE_UPPER_LIMIT(F_Xo))
 ...
 * @{
 */
#define DATARATE_DIVIDER                 268435456          /*!< 2^28 factor dividing fxo in data rate formula */


#define DATARATE_EXPONENT_UPPER_LIMIT    15  /*!< Maximum value for the exponent in datarate formula */

#define MINIMUM_DATARATE                 100  /*!< Minimum datarate supported by SPIRIT1 100 bps */
#define MAXIMUM_DATARATE                 500000  /*!< Maximum datarate supported by SPIRIT1 500 kbps */

#define IS_DATARATE_EXP(EXP)            (EXP>=0 && EXP<=DATARATE_EXPONENT_UPPER_LIMIT)
...
#define FDEV_DIVIDER                    262144             /*!< 2^18 factor dividing fxo in fdev formula */
...
#define F_DEV_LOWER_LIMIT(F_Xo)		(F_Xo*4.0/FDEV_DIVIDER)
#define F_DEV_UPPER_LIMIT(F_Xo)		(F_Xo*3584.0/FDEV_DIVIDER)

#define IS_FDEV_EXP(EXP)                (EXP>=0 && EXP<=F_DEV_EXPONENT_UPPER_LIMIT)
#define IS_FDEV_MANT(MANT)              (MANT>=0 && MANT<=F_DEV_MANTISSA_UPPER_LIMIT)
...
#define BW_EXPONENT_UPPER_LIMIT   9  /*!< Maximum value for the exponent in bandwidth table */
#define BW_MANTISSA_UPPER_LIMIT   8  /*!< Maximum value for the mantissa in bandwidth table */

#define CH_BW_LOWER_LIMIT(F_Xo)      1100.0*(F_Xo/26e6)  /*!< Minimum value of the channel filter bandwidth */
#define CH_BW_UPPER_LIMIT(F_Xo)    800100.0*(F_Xo/26e6)  /*!< Maximum value of the channel filter bandwidth */

#define IS_BW_EXP(EXP)       ((EXP)>=0 && (EXP)<=BW_EXPONENT_UPPER_LIMIT)
#define IS_BW_MANT(MANT)     ((MANT)>=0 && (MANT)<=BW_MANTISSA_UPPER_LIMIT)

#define IS_CH_BW(BW,F_Xo)         ((BW)>=CH_BW_LOWER_LIMIT(F_Xo) && (BW)<=CH_BW_UPPER_LIMIT(F_Xo))

#define IS_CHBW_DATARATE(CHBW,DATARATE) (CHBW>=(2*DATARATE))
...
#define IS_PAPOWER_DBM(PATABLE)      ((PATABLE)>= PA_LOWER_LIMIT_DBM && (PATABLE)<=PA_UPPER_LIMIT_DBM)
...
void SpiritRadioSetDatarate(float lDatarate);
float SpiritRadioGetDatarate(void);
void SpiritRadioSetFrequencyDev(float lFDev);
