...
void SpiritTimerGetRxTimeout(float* pfTimeoutMsec, uint8_t* pcCounter , uint8_t* pcPrescaler)
{
  ...
  {
    *pfTimeoutMsec = (float)(((tempRegValue[0]+1)*tempRegValue[1]+1)*(float)(RX_TCLK_26MHz/1000));
  }
  else
  {
    *pfTimeoutMsec = (float)(((tempRegValue[0]+1)*tempRegValue[1]+1)*(float)(RX_TCLK_24MHz/1000));
  }
